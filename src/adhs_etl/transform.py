...
class FieldMapper:
    ...
    def map_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for target, source_cols in target_columns.items():
            if len(source_cols) > 1:
                ...
                logger.info(
                    f"For target '{target}', chose source column '{chosen_col}' from {source_cols}"
                )
            else:
                # Single mapping
    ...
    def save_unknown_columns(self, dry_run: bool = False) -> None:
        ...
        if not dry_run:
            ...
            logger.info(
                f"Updated {self.field_map_todo_path} with {len(self.unknown_columns)} unknown columns"
            )
...
def save_dataframes_to_excel(
    dataframes: Dict[str, pd.DataFrame], output_path: Path, dry_run: bool = False
) -> None:
