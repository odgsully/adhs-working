...
def get_previous_month_data(
    all_months_dir: Path, current_month: int, current_year: int
) -> pd.DataFrame:
    ...
    if prev_folder.exists():
        ...
        field_mapper = EnhancedFieldMapper(
            Path("field_map.yml"), Path("field_map.TODO.yml")
        )
        provider_grouper = ProviderGrouper()

        return process_month_data(
            prev_folder, field_mapper, provider_grouper, prev_month, prev_year
        )
...
def run(
    ...
    logger.info(
        f"Starting enhanced ADHS ETL pipeline for month: {month_num}/{year_num}"
    )
    if dry_run:
    ...
    field_mapper = EnhancedFieldMapper(
        settings.field_map_path, settings.field_map_todo_path
    )
    ...
    if not dry_run:
        reformat_path = create_reformat_output(
            current_month_df, month_num, year_num, reformat_dir
        )
    ...
    if not dry_run:
        all_to_date_path = create_all_to_date_output(
            current_month_df, month_num, year_num, all_to_date_dir, batch_size
        )
    ...
    analysis_df = analyzer.analyze_month_changes(
        current_month_df, previous_month_df, all_historical_df
    )
    ...
    if not all_historical_df.empty:
        months_data = analyzer.create_monthly_counts(
            all_historical_df, month_num, year_num
        )
    ...
    api_batch_size = max(
        5, batch_size // 100
    )  # Scale API batch size based on processing batch size
    ...
    for i in range(0, len(seller_indices), api_batch_size):
        ...
        logger.info(
            f"Processing MCAO batch {i//api_batch_size + 1}/{(len(seller_indices) + api_batch_size - 1)//api_batch_size}"
        )
        ...
        for idx in batch_indices:
            ...
            try:
                ...
                if property_data:
                    ...
                    analysis_df.loc[idx, "STORIES"] = property_data.get(
                        "stories", "N/A"
                    )
                    analysis_df.loc[idx, "OWNER NAME"] = property_data.get(
                        "owner_name", "N/A"
                    )
                    analysis_df.loc[idx, "OWNER MAILING"] = property_data.get(
                        "owner_mailing", "N/A"
                    )
                    analysis_df.loc[idx, "PURCHASE PRICE"] = property_data.get(
                        "purchase_price", "N/A"
                    )
                    analysis_df.loc[idx, "PURCHASE DATE"] = property_data.get(
                        "purchase_date", "N/A"
                    )
            except Exception as e:
