...
from adhs_etl.transform_enhanced import EnhancedFieldMapper, ProviderGrouper
...
class TestCapacityMapping:
    ...
    def test_capacity_numeric_default(self):
        ...
        test_data = {"PROVIDER": ["Test Provider"], "ADDRESS": ["123 Main St"]}

        _df = pd.DataFrame(test_data)  # noqa: F841 - test fixture

        # When CAPACITY is missing, it should get pd.NA (not empty string)
        _expected_default = pd.NA  # noqa: F841 - expected value for future assertions
