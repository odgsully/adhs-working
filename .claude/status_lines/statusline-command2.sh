...
# Get git commit time (suppress errors)
git_commit_time=""
if git -C "$current_dir" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    # Get the timestamp of the last commit
    last_commit_timestamp=$(git -C "$current_dir" log -1 --format=%ct 2>/dev/null)
    if [ -n "$last_commit_timestamp" ]; then
        ...
        if [ "$days_diff" -eq 0 ]; then
            git_commit_time="today"
        elif [ "$days_diff" -eq 1 ]; then
            git_commit_time="1 day ago"
        else
            git_commit_time="${days_diff} days ago"
        fi
        
        # Add color coding based on days (white to maroon gradient)
        # Maroon color codes: lighter to darker maroon
        if [ "$days_diff" -eq 0 ]; then
            # Today - white
            git_commit_time="\033[97m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 1 ]; then
            # 1 day - very light maroon
            git_commit_time="\033[38;5;217m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 2 ]; then
            # 2 days - light maroon
            git_commit_time="\033[38;5;210m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 3 ]; then
            # 3 days
            git_commit_time="\033[38;5;203m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 4 ]; then
            # 4 days
            git_commit_time="\033[38;5;196m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 5 ]; then
            # 5 days
            git_commit_time="\033[38;5;160m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 6 ]; then
            # 6 days
            git_commit_time="\033[38;5;124m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 7 ]; then
            # 7 days
            git_commit_time="\033[38;5;88m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 8 ]; then
            # 8 days
            git_commit_time="\033[38;5;52m$git_commit_time\033[0m"
        elif [ "$days_diff" -eq 9 ]; then
            # 9 days
            git_commit_time="\033[38;5;53m$git_commit_time\033[0m"
        else
            # 10+ days - full maroon
            git_commit_time="\033[38;5;88m$git_commit_time\033[0m"
        fi
...
if [ -n "$git_commit_time" ]; then
    output="$output | $git_commit_time"
fi
