...
> Asynchronous endpoint for bulk phone number validation with webhook delivery

## Overview

| Attribute | Value |
|-----------|-------|
| **Endpoint** | `POST https://api.batchdata.com/api/v1/phone/verification/async` |
| **Cost** | $0.007 per phone number |
| **Rate Limit** | See [BatchData API Rate Limits](https://developer.batchdata.com/docs/batchdata/rate-limits) |
| **Documentation** | https://developer.batchdata.com/docs/batchdata/batchdata-v1/operations/create-a-phone-verification-async |

## Description

This is the asynchronous version of the Phone Verification API endpoint.
...
3. **Payload Format**: The webhook payload contains the complete response matching the synchronous Phone Verification endpoint format

**Recommended for**: Processing large batches of phone verification requests.
...
A list of phone numbers for which you want detailed information.
...
Complete verification results delivered to your webhook URL.
...
```bash
curl --request POST \
  --url https://api.batchdata.com/api/v1/phone/verification/async \
...
- [Phone DNC Async](phone-dnc-async.md) - Do-Not-Call registry check
